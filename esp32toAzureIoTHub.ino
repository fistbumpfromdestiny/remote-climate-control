...
float humidity;
float temperature;
float thresholds[2] = {40.0, 60.0};

int remote_status = AUTO;
...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
  ...
  switch (event->event_id)
  {
    ...
    case MQTT_EVENT_DATA:
      ...
      if (String(incoming_data) == "True") {
        remote_status = ON;
        }
      else if (String(incoming_data) == "False") {
        remote_status = OFF;
        }
      else if (String(incoming_data) == "Auto") {
        remote_status = AUTO;
        }
      
      else {
        incoming = strtok(incoming_data, " ");
  
        while (incoming != NULL) {
          Serial.println("Old Threshold: " + String(thresholds[idx]));
          thresholds[idx] = atoi(incoming);
          Serial.println("New Threshold: " + String(thresholds[idx]));
          incoming = strtok(NULL, " ");
          idx++;
          }
      }
      break;
...
static void getTelemetryPayload(az_span payload, az_span* out_payload)
{
  ...
  float threshold1 = thresholds[0];
  float threshold2 = thresholds[1];
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR("{ \"humiditiy\": "));
  (void)az_span_u32toa(payload, humidity, &payload);
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", \"temperature\": "));
  (void)az_span_u32toa(payload, temperature, &payload);
   payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", \"current_lower_threshold\": "));
  (void)az_span_u32toa(payload, threshold1, &payload);
   payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", \"current_upper_threshold\": "));
  (void)az_span_u32toa(payload, threshold2, &payload);
...
// Esp32 setup and loop main functions.
...
void loop()
{
  ...
  if(remote_status == ON && !digitalRead(5)) {
    digitalWrite(5, HIGH);
  }
  else if(remote_status == OFF && digitalRead(5)) {
    digitalWrite(5, LOW);
  }
  
  else if(remote_status == AUTO){
    if(humidity < thresholds[0] && !digitalRead(5)){
      Logger.Info("Activating mister.");
      digitalWrite(5, HIGH);
    }
   
    else if (humidity > thresholds[1] && digitalRead(5)) {
      Logger.Info("Deactivating mister.");
      digitalWrite(5, LOW);
    }
  }
